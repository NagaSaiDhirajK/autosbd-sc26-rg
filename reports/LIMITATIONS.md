...
- Hot selector overhead is measured for both the preliminary Fe₄S₄ deployment tree and the current all-15 multifamily deployment tree. The current multifamily object-cold median is `840.905 us`; it includes model-file read/parse with uncontrolled OS page-cache state and must not be described as storage-cache-cold latency. These are single-session microbenchmarks on one node, not latency distributions across machines or cold storage states.
