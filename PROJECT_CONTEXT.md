...
Current phase: Stages 0–4 and the current single-family Stage 5 evaluation are complete. The frozen Stage 4 protocol produced 48 immutable records, including 38 timing-eligible measurements, through the official AMD CPU16 and NVIDIA OpenMP-offload L4 executables built from the same unmodified commit with NVIDIA HPC SDK 26.5. Corrected Stage 5 artifacts evaluate six unique policies with grouped, training-only fits and no invalid selection or failure; hot and diagnostic cold inference overhead are measured separately. Phase B1, the Phase B2 ten-input schema-v3 correctness gate, and the bounded 40-record N₂/H₂O pilot are complete. The pilot has 20 excluded warmups and 20 eligible single-repetition measurements and brackets a CPU/GPU flip between 3,025 and 10,000 configurations in both families. A 104-record repeated protocol is frozen and awaiting explicit approval; multifamily evaluation remains pending. The RIKEN checkout supplied data only; its executable remains historical and is not a primary or comparison backend.
...
The current suite passes all 168 tests. Coverage additionally includes deterministic N₂/H₂O prefixes, config-v2/raw-v3 family identity, family-aware calibration pairing and timing aggregation, strict external augmentation of all 48 immutable Fe₄S₄ records, the completed 40-record manifest-linked Phase B pilot, the frozen 104-record final shard geometry/protocol hashes, and stop-on-first-non-success orchestration. The definitive Fe₄S₄ five-size calibration validates ten clean CPU/GPU correctness records; Phase B1 preserves four compatibility records; and the homogeneous Phase B2 manifest validates 20 clean family-aware records across ten exact inputs. Stage 4 preserves 48 raw records and aggregates only its 38 eligible measurements; Stage 5 deliberately uses a balanced 30-measurement view for held-out evaluation.
...
- Phase B: B1, the homogeneous all-v3 ten-input correctness gate, and the 40-record manifest-linked pilot are complete. The final 104-record protocol is frozen from pilot evidence and remains blocked on explicit approval; multifamily evaluation follows repeated timing.
