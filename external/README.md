...
| `external/riken-sbd` | https://github.com/r-ccs-cms/sbd | tag `v1.3.0`, commit `b71e1c3ed857fcb4fb05731dc285831c1afe9ebd` | Apache-2.0; RIKEN notice | Secondary source preserving prior CUDA/Thrust build, input, and smoke-test evidence |
...
## AMD primary path and historical RIKEN contingency
...
The checked-in CPU configuration is a Homebrew/macOS example, and the checked-in GPU comments use NVIDIA HPC SDK. AutoSBD changes no upstream file. `scripts/build_upstream.sh` supplies recorded Linux/GCC flags for CPU and directly compiles the Thrust source with CUDA 12.9 nvcc for `sm_89`.
...
This RIKEN build and its smoke evidence remain reproducible historical provenance, but they do not replace AMD-HPC as AutoSBD's primary benchmark backend.
