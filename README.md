...
Stages 0–2 are complete. Stage 3 workload preparation, five-size CPU/GPU correctness calibration, the bounded CPU16/GPU crossover pilot, and CPU-thread pruning are also complete: the machine and upstream builds are documented, identical-input AMD CPU/GPU correctness passes at every calibrated size, and the schema-v2 single-run/resumable-sweep harness is implemented. The current suite has 97 passing tests, including the frozen Stage 4 protocol, deterministic timing aggregation, and Stage 3 workload, calibration-manifest, multi-input, and clean-resume coverage.
...
The one-repetition Stage 3 pilot brackets the observed CPU16/GPU crossover between 1,024 and 3,025 configurations. The follow-up CPU-thread pilot found CPU1/4/8 dominated by CPU16 at every tested size, so the final axis is frozen to CPU16 and the L4 GPU. These are pilot decisions without uncertainty, not final speedup results. The repeated Stage 4 protocol is frozen but not yet run; held-out evaluation and the runtime selector have not yet been completed.
