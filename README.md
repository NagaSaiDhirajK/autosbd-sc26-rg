...
The schema-v2 Fe₄S₄ CPU and GPU runs both converged and agreed in energy and density. Their recorded wall times are diagnostic correctness-smoke observations only: both have `timing_eligible=false`, so they are not final performance results. See [`reports/RESULTS.md`](reports/RESULTS.md) and [`reports/LIMITATIONS.md`](reports/LIMITATIONS.md).

Stage 3 is workload preparation and a bounded geometric pilot. Final timing, held-out evaluation, and the runtime selector have not yet been completed.
