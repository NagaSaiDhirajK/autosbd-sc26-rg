...
Stages 0–2 are complete. Stage 3 workload preparation, five-size CPU/GPU correctness calibration, and the bounded CPU16/GPU pilot are also complete: the machine and upstream builds are documented, identical-input AMD CPU/GPU correctness passes at every calibrated size, and the schema-v2 single-run/resumable-sweep harness is implemented. The current suite has 96 passing tests, including deterministic timing aggregation and Stage 3 workload, calibration-manifest, multi-input, and clean-resume coverage.
...
The one-repetition Stage 3 pilot brackets the observed CPU16/GPU crossover between 1,024 and 3,025 configurations. This is a pilot locator without uncertainty, not a final speedup result. A small CPU-thread pilot is configured next; repeated final timing, held-out evaluation, and the runtime selector have not yet been completed.
